...
def Finder_IETE(time1, time_name1, start1):
    ...
    if (C - B) < (C - D) and (C - B) < (E - D) and (E - F) < (E - D) and (E - F) < (
        ...
        if I[1] > B and J[
            ...
            pourcentage_10_ep1 = (10 * (local_max[-2] - I[0])) / 100
            pourcentage_10_ep2 = (10 * (J[0] - local_max[-1])) / 100
            ...
            plt.plot([], [], ' ')
            plt.title(f'IETE : {tiker_live} | {time1} {time_name1} | +{pourcent_chercher}% | -{pourcent_perdu}% | {note}/10 | {debugage}', fontweight="bold", color='black')
